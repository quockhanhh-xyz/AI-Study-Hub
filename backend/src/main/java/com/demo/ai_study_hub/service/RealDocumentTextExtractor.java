...
public class RealDocumentTextExtractor implements DocumentTextExtractor {
    ...
    private static final Set<String> SUPPORTED_TYPES = Set.of("PDF", "TXT", "DOCX", "PPTX", "XLSX");
    ...
    public ExtractionResult extract(Document doc) {
        ...
        try {
            if ("PDF".equals(fileType)) {
                ...
                DocxTextExtractor.DocxExtractionResult docxResult = docxTextExtractor.extract(fileBytes);
                ...
                TextCleaner.CleanResult cleaned = textCleaner.clean(docxResult.rawText);
                ...
                PptxTextExtractor.PptxExtractionResult pptxResult = pptxTextExtractor.extract(fileBytes);
                ...
                TextCleaner.CleanResult cleaned = textCleaner.clean(pptxResult.rawText);
                if (cleaned.cleanedText == null || cleaned.cleanedText.isBlank()) {
                    return ExtractionResult.builder()
                            .status(ProcessingStatus.EMPTY_CONTENT)
                            .error("No extractable text found in document")
                ...
                XlsxTextExtractor.XlsxExtractionResult xlsxResult = xlsxTextExtractor.extract(fileBytes);
                ...
                TextCleaner.CleanResult cleaned = textCleaner.clean(xlsxResult.rawText);
                if (cleaned.cleanedText == null || cleaned.cleanedText.isBlank()) {
                    return ExtractionResult.builder()
                            .status(ProcessingStatus.EMPTY_CONTENT)
                            .error("No extractable text found in document")
