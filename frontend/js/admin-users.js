...
function renderActionButtons(user) {
    ...
    if (user.status === 'BLOCKED') {
        return `<button class="btn btn-outline" style="padding: 4px 8px; font-size: 12px;" onclick="promptUpdateStatus(${user.userId}, 'ACTIVE', 'Unblock this user?')">Unblock</button>`;
    } else {
        return `<button class="btn btn-outline" style="padding: 4px 8px; font-size: 12px; color: var(--danger); border-color: var(--danger);" onclick="promptUpdateStatus(${user.userId}, 'BLOCKED', 'Block this user?')">Block</button>`;
    }
}
