/**
 * Admin Profile and Security settings manager
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function populateProfileForm(profile) {
    ...
    systemRole.textContent = "Administrator";
  ...
  async function loadUserProfile() {
    try {
      ...
      if (response && response.data) {
        ...
        showStatus("Failed to load admin profile data", "error");
      }
    } catch (e) {
      console.error("Failed to load profile details:", e);
      showStatus("Failed to load admin profile details from server", "error");
    }
  }
  ...
  if (profileForm) {
    profileForm.addEventListener("submit", async (e) => {
      ...
      // Strict contract DTO matching: only send admin payload fields
      const profilePayload = {
        fullName: fullNameVal,
        phone: phoneInput.value.trim() || null,
        bio: bioInput.value.trim() || null,
        // Send other fields as null/empty to preserve profile schema mapping
        schoolName: currentProfileData?.schoolName || null,
        major: currentProfileData?.major || null,
        studentCode: currentProfileData?.studentCode || null,
        graduationYear: currentProfileData?.graduationYear || null,
        educationLevel: currentProfileData?.educationLevel || null
      };
      ...
      try {
        ...
        if (response && response.data) {
          ...
          showStatus("Admin profile details updated successfully!", "success");
        } else {
  ...
  if (avatarInput) {
    avatarInput.addEventListener("change", async (e) => {
      ...
      try {
        showStatus("Uploading admin avatar photo...", "checking");
        ...
        if (response && response.data) {
          ...
          showStatus("Admin avatar uploaded successfully!", "success");
        } else {
  ...
  if (passwordForm) {
    passwordForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        showStatus("Admin password changed successfully!", "success");
      } catch (err) {
