...
async function loadDashboardData(isManualRefresh = false) {
  ...
  try {
    const days = document.getElementById("chartDateRange")
      ? parseInt(document.getElementById("chartDateRange").value)
      : 30;
    ...
    if (
      ...
      const autoRefreshText = document.getElementById(
        "dashboardAutoRefreshText",
      );
    ...
    if (!isManualRefresh) {
      ...
      document.getElementById("dashboardErrorMessage").textContent =
...
function renderDashboardStats(data) {
  ...
  document.getElementById("statTotalDocs").textContent = (
    data.totalDocuments || 0
  ).toLocaleString();
  document.getElementById("statPendingDocs").textContent = (
    data.pendingPublicDocuments || 0
  ).toLocaleString();
  document.getElementById("statAiRequests").textContent = (
    data.aiRequestsToday || 0
  ).toLocaleString();

  const revenue = data.lifetimeRevenue || 0;
  document.getElementById("statTotalRevenue").textContent =
    formatAdminRevenue(revenue);
  ...
  if (successPaymentsEl)
    successPaymentsEl.textContent = `${successPayments.toLocaleString()} successful payments`;
...
function renderDashboardCharts(data, summaryData = null) {
  ...
  renderChart(
    "chartUsersByTier",
    "chartUsersByTierContainer",
    "doughnut",
    usersByTier,
    "tier",
    "count",
    ["#64748b", "#3b82f6", "#8b5cf6"],
    "No users found.",
  );
  ...
  renderChart(
    "chartDocsByStatus",
    "chartDocsByStatusContainer",
    "pie",
    docsByStatus,
    "approvalStatus",
    "count",
    ["#f59e0b", "#16a34a", "#ef4444", "#64748b"],
    "No documents found.",
  );
  ...
  const formattedRevenue = revenueByDay.map((i) => ({
    ...i,
    date: formatDate(i.date),
  }));
  renderChart(
    "chartRevenue",
    "chartRevenueContainer",
    "line",
    formattedRevenue,
    "date",
    "revenue",
  ...
  const formattedAiUsage = aiUsageByDay.map((i) => ({
    ...i,
    date: formatDate(i.date),
  }));
  renderChart(
    "chartAiUsage",
    "chartAiUsageContainer",
    "bar",
    formattedAiUsage,
    "date",
    "count",
...
function renderChart(
  ...
  if (
    ...
    ((type === "pie" || type === "doughnut") &&
      dataArray.every((item) => item[dataKey] === 0))
  ) {
    ...
    plugins: [
      {
        ...
        beforeDraw: function (chart) {
          ...
          for (let i = 0; i < dataValues.length; i++) {
            ...
            if (
              dataLabels[i].toUpperCase().includes("PRO") ||
              dataLabels[i].toUpperCase().includes("PREMIUM")
            ) {
          ...
          if (targetValue === 0 && dataValues.length > 0) {
            ...
            targetLabel = dataLabels[index]
              ? dataLabels[index].split(" ")[0].toUpperCase()
              : "TOTAL";
          }

          const percent =
            total > 0 ? Math.round((targetValue / total) * 100) : 0;
