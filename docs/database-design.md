...
- **Summary Status Auditing**: The `status` column in `ai_summaries` can take `SUCCESS` or `FAILED`. In the Step 14 MVP, failed generation attempts (due to AI output invalid or provider error) do not write to the DB. However, the `FAILED` value is retained for future extension where attempts may be persisted for audit purposes.
