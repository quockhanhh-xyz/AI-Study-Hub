...
|---|---|---|---|---|
| TC-PAY-001 | Guest | No auth cookies | 200 OK | Returns list of plans containing `FREE` and `PREMIUM` details. |
| TC-PAY-002 | Current User | Authenticated | 200 OK | Returns plans. `FREE` plan must have `price = 0`, `billingLabel = "free"`, and `aiDailyLimit = 5`. `PREMIUM` plan must have `price = 199000 VND`, `billingLabel = "month"`, and `aiDailyLimit = 50`. |
...
| TC-PAY-011 | FREE User | `"PREMIUM"` | 200 OK | Creates a new order. `status = "PENDING"`, `amount = 199000`, `currency = "VND"`, `billingLabel = "month"`, `paymentMethod = "MOCK"`. |
| TC-PAY-012 | FREE User | `"FREE"` | 400 Bad Request | Plan code `FREE` cannot be upgraded to (already default or cannot purchase). |
...
| TC-PAY-023 | Upgraded User | Payment order already `SUCCESS` | 409 Conflict | No changes. Response returns `status = SUCCESS`, user remains `PREMIUM`. |
...
| TC-PAY-026 | Current User | Payment order `PENDING`, owned by current user | 200 OK | User tier becomes `PREMIUM` atomically. Order status becomes `SUCCESS`. `paidAt` is set to the current timestamp. |
| TC-PAY-027 | Upgraded User | Payment order `PENDING` (created when user was FREE, user upgraded since then) | 409 Conflict | Blocked from completing. Already PREMIUM users cannot confirm old pending payments. |
| TC-PAY-028 | Current User | Double-click: 2 concurrent requests on the same `PENDING` payment ID | 200 OK / 409 Conflict | First request succeeds (200 OK, upgrades user, sets `SUCCESS`). Second request returns 409 Conflict (already SUCCESS). |
...
| TC-PAY-032 | Upgraded User | Payment order already `SUCCESS` | 409 Conflict | No changes. User tier remains `PREMIUM`. |
| TC-PAY-033 | Current User | Payment order `PENDING`, owned by current user | 200 OK | Order status becomes `FAILED`. User tier remains `FREE`. `paidAt` remains `null`. Response returns `tier = "FREE"`. |
| TC-PAY-034 | Upgraded User | Payment order `PENDING` (from when user was FREE) | 200 OK | Order status becomes `FAILED`. User tier remains `PREMIUM` (already upgraded by other means). `paidAt` remains `null`. Response returns `tier = "PREMIUM"`. |
...
| TC-PAY-042 | Upgraded User | Payment order already `SUCCESS` | 409 Conflict | No changes. User tier remains `PREMIUM`. |
| TC-PAY-043 | Current User | Payment order `PENDING`, owned by current user | 200 OK | Order status becomes `CANCELLED`. User tier remains `FREE`. `paidAt` remains `null`. Response returns `tier = "FREE"`. |
| TC-PAY-044 | Upgraded User | Payment order `PENDING` (from when user was FREE) | 200 OK | Order status becomes `CANCELLED`. User tier remains `PREMIUM`. `paidAt` remains `null`. Response returns `tier = "PREMIUM"`. |
